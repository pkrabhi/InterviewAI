...
export default function ReportScreen({ route, navigation }) {
  ...
  const fetchReport = async () => {
    try {
      ...
      const strengths    = JSON.parse(data.strengths || '[]');
      const improvements = JSON.parse(data.improvements || '[]');
      const nextTopics   = JSON.parse(data.nextTopics || '[]');
      setReport({ ...data, strengths, improvements, nextTopics });
    } catch (error) {
      // Report may still be generating — retry up to 5 times
      if (retries < 5) {
        setTimeout(() => {
          setRetries((r) => r + 1);
          fetchReport();
